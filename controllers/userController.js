...
import jwt from "jsonwebtoken";
 ...
 let userController = async (req, res) => {
  ...
  try {
    ...
    return res.status(201).send({ status: "Success", message: "Registration successfull", "token" : token });
        ...
        if(email && password){
            ...
            if (user != null){
                ...
                if(user.email === email && isMatch){
                    ...
                    res.status(400).send({ status: "Failed", message: "Password or Email Doesn't match" });
                }
...
export let changeUserPassword = async (req,res) => {
    ...
    if (password && password_confirmation){
        if(password !== password_confirmation){
            res.status(400).send({ status: "Failed", message: "New password and Confrim password Doesn't Match" });
        }else{
           const salt = await bcrypt.genSalt(10)
